// Create sub-services for the existing ITR-1 service
const axios = require('axios');

const LOCAL_API_BASE_URL = 'http://localhost:8080/api/v1';

async function createSubServices() {
  console.log('🔧 Creating Sub-Services for ITR-1...\n');
  
  try {
    // First, get the existing service ID
    console.log('1️⃣ Getting existing service...');
    ...
    // Create sub-services for ITR-1
    console.log('\n2️⃣ Creating sub-services...');
    
    const subServices = [
      {
        serviceId: itrService._id,
    ...
    for (const subService of subServices) {
      try {
        const response = await axios.post(`${LOCAL_API_BASE_URL}/sub-services`, subService);
