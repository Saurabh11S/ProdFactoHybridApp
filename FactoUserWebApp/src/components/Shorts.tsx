...
export function Shorts({}: ShortsProps) {
  ...
  const handlePrevious = useCallback(() => {
    ...
    isTransitioning.current = true;
    setCurrentShort(
      (prev) => (prev - 1 + shorts.length) % shorts.length
    );
    setTimeout(() => {
      isTransitioning.current = false;
    }, 300);
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-[#F9FAFB]'}`} style={{ paddingTop: '70px' }}>
      <div className={`max-w-md mx-auto h-[calc(100vh-70px)] relative overflow-hidden ${isDarkMode ? 'bg-gray-900' : 'bg-[#F9FAFB]'}`}>
              ...
              >
                ...
                {shouldLoad && (
                  <>
                    ...
                    {isActive && (
                      <div className="absolute bottom-0 left-0 p-4 pb-20 z-30 max-w-[60%]">
                    ...
                    {isActive && (
                      <div className="absolute right-3 bottom-20 flex flex-col items-center space-y-5 z-30">
                        <button 
                          onClick={async (e) => {
                            e.stopPropagation();
                            try {
                              ...
                              const shareText = short.description || `Watch: ${shareTitle}`;
        ...
        <div className="absolute top-0 left-0 right-0 flex space-x-1 px-2 pt-2 z-50">
          {shorts.map((_, index) => (
            <div
              key={index}
